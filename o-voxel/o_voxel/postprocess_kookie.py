...
# MIT license
#
# Modified from TRELLIS.2 (https://github.com/microsoft/TRELLIS.2/blob/main/o-voxel/o_voxel/postprocess.py)
...
def to_glb(
    ...
    # [kookie] Safety check: skip fill_holes for small meshes to avoid CuMesh CUDA errors
    # Small meshes (e.g., thin objects, individual assets) can cause CuMesh to fail
    MIN_VERTS_FOR_FILL_HOLES = 10000  # Threshold for fill_holes operation
    if mesh.num_vertices >= MIN_VERTS_FOR_FILL_HOLES:
        mesh.fill_holes(max_hole_perimeter=3e-2)
        if verbose:
            print(f"After filling holes: {mesh.num_vertices} vertices, {mesh.num_faces} faces")
    else:
        if verbose:
            print(f"[Skip] Initial fill_holes skipped (mesh has {mesh.num_vertices} verts, threshold is {MIN_VERTS_FOR_FILL_HOLES})")
    ...
    if not remesh:
        # [kookie] Safety check: skip simplification if mesh is already small enough
        # This prevents CuMesh from failing on small assets where decimation_target > num_vertices
        current_verts = mesh.num_vertices

        # Step 1: Aggressive simplification (3x target) - only if mesh is large enough
        # [kookie] Original: mesh.simplify(decimation_target * 3, verbose=verbose)
        initial_target = min(decimation_target * 3, current_verts)
        if current_verts > decimation_target * 3:
            mesh.simplify(initial_target, verbose=verbose)
            if verbose:
                print(f"After inital simplification: {mesh.num_vertices} vertices, {mesh.num_faces} faces")
        else:
            if verbose:
                print(f"[Skip] Initial simplification skipped (mesh has {current_verts} verts, target was {decimation_target * 3})")

        # Step 2: Clean up topology (duplicates, non-manifolds, isolated parts)
        mesh.remove_duplicate_faces()
        mesh.repair_non_manifold_edges()
        mesh.remove_small_connected_components(1e-5)
        # [kookie] Original: mesh.fill_holes(max_hole_perimeter=3e-2)
        if mesh.num_vertices >= MIN_VERTS_FOR_FILL_HOLES:
            mesh.fill_holes(max_hole_perimeter=3e-2)
        if verbose:
            print(f"After initial cleanup: {mesh.num_vertices} vertices, {mesh.num_faces} faces")

        # [kookie] Update current vertex count after cleanup
        current_verts = mesh.num_vertices

        # Step 3: Final simplification to target count - only if mesh is large enough
        # [kookie] Original: mesh.simplify(decimation_target, verbose=verbose)
        if current_verts > decimation_target:
            ...
            if verbose:
                print(f"[Skip] Final simplification skipped (mesh has {current_verts} verts, target was {decimation_target})")
        ...
        # [kookie] Original: mesh.unify_face_orientations()
        # Skip for small meshes to avoid CuMesh CUDA errors (illegal memory access)
        if mesh.num_vertices >= MIN_VERTS_FOR_FILL_HOLES:
            mesh.unify_face_orientations()
        else:
            if verbose:
                print(f"[Skip] unify_face_orientations skipped (mesh has {mesh.num_vertices} verts)")
