...
class SparseStructureFlowModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor, cross_attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            x: [N, C, D, H, W] input latent tensor
            t: [N] timestep tensor
            cond: [N, L_ctx, C_ctx] conditioning tensor (e.g., image features)
            cross_attn_mask: [N, L, L_ctx] optional attention mask for spatially-aware cross-attention.
                            Values of 0 mean attend, large negative values mean don't attend.
                            L = D*H*W (flattened voxel positions), L_ctx = number of condition tokens.

        Returns:
            [N, C, D, H, W] output tensor
        """
        ...
        if cross_attn_mask is not None:
            cross_attn_mask = manual_cast(cross_attn_mask, self.dtype)
        for block in self.blocks: # self.blocks: ModuleList[ModulatedTransformerCrossBlock] x 30
            h = block(h, t_emb, cond, self.rope_phases, cross_attn_mask) # block: ModulatedTransformerCrossBlock #NOTE: added cross_attn_mask
