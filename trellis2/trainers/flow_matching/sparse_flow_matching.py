...
class SparseFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        noise = x_0.replace(torch.randn_like(x_0.feats))    # fixed x_0.coords and replace noise with same shape, x_0.feats -> x_0.coords.shape = [10837, 4], x_0.feats.shape = [10837, 33]
        t = self.sample_t(x_0.shape[0]).to(x_0.device).float()
        x_t = self.diffuse(x_0, t, noise=noise)             # noisy input
        cond = self.get_cond(cond, **kwargs)                # cond.shape = [1, 6174, 1024]

        pred = self.training_models['denoiser'](x_t, t * 1000, cond, **kwargs)
        assert pred.shape == noise.shape == x_0.shape       # predict velocity
        target = self.get_v(x_0, noise, t)                  # GT velocity
        terms = edict()
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            # Save coords before deleting x_0 (needed for per-voxel spatial masks in Stage 2)
            x_0_coords = batch_data['x_0'].coords
            del batch_data['x_0']
            # Remove non-tensor metadata fields before inference
            for _meta_key in ['_data_path', 'sample_path', 'asset_names', 'n_visible_assets']:
                batch_data.pop(_meta_key, None)
            # Pass coords for per-voxel spatial mask (only used by asset-aware trainers)
            if 'part_layouts' in batch_data:
                batch_data['x_0_coords'] = x_0_coords
            args = self.get_inference_cond(**batch_data) # args['cond'].shape = [4, 6174, 1024]
            res = sampler.sample(
        ...
        if self.eval_dataset is not None:
            ...
            eval_cond_vis = []
            for i in range(0, num_samples, batch_size):
                batch_data = {k: v[i:i+batch_size] for k, v in eval_data.items()}
                batch_data = recursive_to_device(batch_data, 'cuda')
                noise = batch_data['x_0'].replace(torch.randn_like(batch_data['x_0'].feats))
                eval_cond_vis.append(self.vis_cond(**batch_data))
                x_0_coords = batch_data['x_0'].coords
                del batch_data['x_0']
                for _meta_key in ['_data_path', 'sample_path', 'asset_names', 'n_visible_assets']:
                    batch_data.pop(_meta_key, None)
            ...
            eval_sample = sp.sparse_cat(eval_sample)

            eval_sample_gt = {k: v for k, v in eval_data.items()}
            eval_sample_pred = {k: v if k != 'x_0' else eval_sample for k, v in eval_data.items()}
            sample_dict['eval_sample_gt'] = {'value': eval_sample_gt, 'type': 'sample'}
            sample_dict['eval_sample'] = {'value': eval_sample_pred, 'type': 'sample'}
