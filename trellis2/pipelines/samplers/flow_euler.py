...
class FlowEulerSampler(Sampler):
    ...
    def sample(
        ...
        **kwargs
        ...
        """
        sample = noise # [1,8,16,16,16] # sample=SparseTensor, sample.coords.shape = [4962, 4], sample.feats.shape = [4962, 32]
        t_seq = np.linspace(1, 0, steps + 1)
        t_seq = rescale_t * t_seq / (1 + (rescale_t - 1) * t_seq)
        t_seq = t_seq.tolist() # len(t_seq) = 13, [1.0, 0.9821428571428572, 0.9615384615384615, 0.9375, 0.9090909090909092, 0.875, 0.8333333333333334, 0.7812500000000001, 0.7142857142857144, 0.625, 0.5000000000000001, 0.3125000000000001, 0.0]
        t_pairs = list((t_seq[i], t_seq[i + 1]) for i in range(steps))
        ret = edict({"samples": None, "pred_x_t": [], "pred_x_0": []})
        for t, t_prev in tqdm(t_pairs, desc=tqdm_desc, disable=not verbose):
            out = self.sample_once(model, sample, t, t_prev, cond, **kwargs)
            sample = out.pred_x_prev # [1,8,16,16,16]
