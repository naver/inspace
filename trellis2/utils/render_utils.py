...
def yaw_pitch_r_fov_to_extrinsics_intrinsics(yaws, pitchs, rs, fovs):
    ...
    for yaw, pitch, r, fov in zip(yaws, pitchs, rs, fovs):
        ...
        # When pitch is near ±90°, up=[0,0,1] is parallel to view direction → degenerate look-at.
        # Use [0,1,0] as up vector in that case to get a well-defined top-down/bottom-up view.
        if abs(pitch.item()) > 1.3:  # ~74.5°, safely before 90°
            up = torch.tensor([0, 1, 0]).float().cuda()
        else:
            up = torch.tensor([0, 0, 1]).float().cuda()
        extr = utils3d.torch.extrinsics_look_at(orig, torch.tensor([0, 0, 0]).float().cuda(), up)
