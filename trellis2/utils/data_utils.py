...
class WeightedResumableSampler(ResumableSampler):
    """
    Distributed sampler that oversamples based on per-sample weights.
    Useful for addressing long-tail distribution (e.g., large rooms are rare).

    Unlike WeightedRandomSampler, this is:
    - Distributed-aware (splits across ranks)
    - Resumable (saves/restores epoch + idx state)
    - Deterministic (seeded by epoch for reproducibility)

    Each epoch, indices are drawn with replacement according to weights,
    producing a full epoch of len(dataset) samples with oversampling.

    Args:
        dataset: Dataset used for sampling. Must have `sample_weights` attribute
            (list of floats, one per sample).
        shuffle: If True (default), apply weighted random sampling.
        seed: Random seed for reproducibility.
        drop_last: Drop tail to make evenly divisible across ranks.
    """
    ...
    def __init__(
        ...
        assert hasattr(dataset, 'sample_weights'), \
            'Dataset must have "sample_weights" attribute to use WeightedResumableSampler'
    ...
    def __iter__(self) -> Iterator:
        ...
        if self.shuffle:
            # Weighted sampling with replacement for one full epoch
