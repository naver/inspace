...
class SparseMultiHeadAttention(nn.Module):
    ...
    def forward(self, x: SparseTensor, context: Optional[Union[VarLenTensor, torch.Tensor]] = None, attn_mask: Optional[torch.Tensor] = None) -> SparseTensor:
        if self._type == "self":
            qkv = self._linear(self.to_qkv, x) # self.to_qkv=Linear(in_features=1536, out_features=4608, bias=True), x.shape = [21, 1536] => qkv.shape = [21, 4608]
            qkv = self._fused_pre(qkv, num_fused=3) # qkv.shape = [21, 3, 12, 128]
            if self.qk_rms_norm or self.use_rope:
                q, k, v = qkv.unbind(dim=-3) # q,k,v.shape = [21, 12, 128]
                if self.qk_rms_norm:
                    q = self.q_rms_norm(q)
                    k = self.k_rms_norm(k)
                if self.use_rope:
                    q, k = self.rope(q, k)
                qkv = qkv.replace(torch.stack([q.feats, k.feats, v.feats], dim=1)) # qkv = SparseTensor(shape=torch.Size([21, 3, 12, 128]), dtype=torch.bfloat16, device=cuda:0)
            if self.attn_mode == "full": # here: self-attention 
            ...
            q = self._linear(self.to_q, x) # [21, 1536]
            q = self._reshape_chs(q, (self.num_heads, -1)) # [21, 12, 128]
            kv = self._linear(self.to_kv, context) # context: [21, 6174, 1024] => kv: [21, 6174, 3072]
            kv = self._fused_pre(kv, num_fused=2) # kv: [21, 6174, 2, 12, 128]
            if self.qk_rms_norm:
                q = self.q_rms_norm(q)
                k, v = kv.unbind(dim=-3) # k: [21, 6174, 12, 128], v: [21, 6174, 12, 128]
                k = self.k_rms_norm(k)
                h = sparse_scaled_dot_product_attention(q, k, v, attn_mask=attn_mask) # attn_mask: [17143, 6174]
            else:
                h = sparse_scaled_dot_product_attention(q, kv, attn_mask=attn_mask) # attn_mask: [17143, 6174]
