...
class ModulatedTransformerCrossBlock(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor, phases: Optional[torch.Tensor] = None, cross_attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            x: [N, L, C] input tensor
            mod: [N, C] modulation tensor (timestep embedding)
            context: [N, L_ctx, C_ctx] context tensor for cross-attention
            phases: [L, C] RoPE phases
            cross_attn_mask: [N, L, L_ctx] attention mask for cross-attention.
        ...
        x = x + h
        h = self.norm2(x)
        h = self.cross_attn(h, context, attn_mask=cross_attn_mask)
        x = x + h
        h = self.norm3(x)
        h = h * (1 + scale_mlp.unsqueeze(1)) + shift_mlp.unsqueeze(1)
        h = self.mlp(h)
        h = h * gate_mlp.unsqueeze(1)
        x = x + h
        return x # [1, 4096, 1536]

    def forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor, phases: Optional[torch.Tensor] = None, cross_attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(self._forward, x, mod, context, phases, cross_attn_mask, use_reentrant=False)
        else:
            return self._forward(x, mod, context, phases, cross_attn_mask)
