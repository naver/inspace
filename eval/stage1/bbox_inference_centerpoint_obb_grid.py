...
OBB-aware + per-sample metrics grid visualization.

In addition to AABB/OBB metrics, this variant dumps a `grid_sampling/` folder
where each grid cell is a (plot, text) pair: bbox plot on the left and per-sample
3D OBB metrics text on the right (F1 / Recall / Precision @ {0.25, 0.50, 0.75},
plus matched (Pred -> GT, IoU, delta-yaw) listing).

Usage:
    python eval/bbox_inference_centerpoint_obb_grid.py \
        ...
        --output_dir evals/bbox_centerpoint_obb_grid \
        --num_vis 20 \
        --score_threshold 0.3
"""
...
# ---------------------------------------------------------------------------
# Per-sample metrics + grid_sampling visualization
# ---------------------------------------------------------------------------

def compute_per_sample_metrics_obb(result, iou_thresholds=(0.25, 0.5, 0.75), scale=None):
    """Compute per-sample 3D OBB metrics for a single sample.

    Args:
        scale: optional float, normalization scale (from `dual_grid_512/normalization_info.json`).
               If provided, center/size errors are also reported in meters.

    Returns dict with:
        n_gt, n_pred:           ints
        per_threshold[t]:       {'tp', 'recall', 'precision', 'f1'}
        matched_pairs:          list of (pred_idx, gt_idx, iou) at lowest threshold
        mean_iou:               mean IoU over matched_pairs
        mean_center_error_3d:   normalized-space L2 center error (mean over matches)
        mean_center_error_3d_m: same but in meters (None if scale not provided)
        mean_size_error:        normalized-space L1 size error (mean over matches)
        mean_size_error_m:      same in meters (None if scale not provided)
        mean_rot_error_deg:     mean abs rotation error in degrees (matches)
        unmatched_gt, unmatched_pred:  counts at lowest threshold
        gt_bboxes, pred_bboxes: [N, 7] arrays (raw OBB values, references)
    """
    iou_thresholds = list(iou_thresholds)
    n_gt = len(result['gt_bboxes'])
    n_pred = len(result['pred_bboxes'])
    ...
    out = {
        ...
        'matched_pairs': [],
        'mean_iou': 0.0,
        'mean_center_error_3d': 0.0,
        'mean_center_error_3d_m': None,
        'mean_size_error': 0.0,
        'mean_size_error_m': None,
        'mean_rot_error_deg': 0.0,
        'unmatched_gt': n_gt,
        'unmatched_pred': n_pred,
        'per_threshold': {t: {'tp': 0, 'recall': 0.0, 'precision': 0.0, 'f1': 0.0}
    ...
    iou_mat = obb_iou_3d_matrix(result['pred_bboxes'], result['gt_bboxes'])

    # Greedy matching at lowest threshold for the matched_pairs listing
    gt_matched_low = np.zeros(n_gt, dtype=bool)
    matched_pairs = []
    center_errors = []
    size_errors = []
    rot_errors = []
    for pi in range(n_pred):
        best_gt = int(iou_mat[pi].argmax())
        iou_val = float(iou_mat[pi, best_gt])
        if iou_val >= iou_thresholds[0] and not gt_matched_low[best_gt]:
            gt_matched_low[best_gt] = True
            matched_pairs.append((pi, best_gt, iou_val))
            pred = result['pred_bboxes'][pi]
            gt = result['gt_bboxes'][best_gt]
            center_errors.append(float(np.linalg.norm(pred[:3] - gt[:3])))
            size_errors.append(float(np.mean(np.abs(pred[3:6] - gt[3:6]))))
            d_yaw = (pred[6] - gt[6] + np.pi) % (2 * np.pi) - np.pi
            rot_errors.append(float(np.abs(d_yaw)))
    out['matched_pairs'] = matched_pairs
    if matched_pairs:
        out['mean_iou'] = float(np.mean([m[2] for m in matched_pairs]))
        out['mean_center_error_3d'] = float(np.mean(center_errors))
        out['mean_size_error'] = float(np.mean(size_errors))
        out['mean_rot_error_deg'] = float(np.degrees(np.mean(rot_errors)))
        if scale is not None and scale > 0:
            out['mean_center_error_3d_m'] = out['mean_center_error_3d'] / scale
            out['mean_size_error_m'] = out['mean_size_error'] / scale
    out['unmatched_gt'] = int(n_gt - len(matched_pairs))
    out['unmatched_pred'] = int(n_pred - len(matched_pairs))

    # Per-threshold metrics (each threshold matched independently)
    for thresh in iou_thresholds:
        gt_matched = np.zeros(n_gt, dtype=bool)
        tp = 0
        for pi in range(n_pred):
...
def format_per_sample_text(ps, iou_thresholds=(0.25, 0.5, 0.75), max_listed_pairs=20,
                            show_box_values=True, max_listed_boxes=12):
    """Build a monospace-friendly multi-line text summary of one sample's 3D OBB metrics."""
    iou_thresholds = list(iou_thresholds)
    width = 36
    lines = []
    lines.append(f'GT: {ps["n_gt"]:<3d}    Pred: {ps["n_pred"]:<3d}')
    lines.append('=' * width)

    lines.append('3D OBB Metrics:')
    for thresh in iou_thresholds:
        m = ps['per_threshold'][thresh]
        lines.append(f'  @{thresh:.2f}  F1={m["f1"]:.3f}  '
                     f'R={m["recall"]:.3f} P={m["precision"]:.3f}  TP={m["tp"]}')

    lines.append('-' * width)

    if ps['matched_pairs']:
        lines.append(f'Matches (n={len(ps["matched_pairs"])}, IoU>={iou_thresholds[0]:.2f}):')
        lines.append(f'  mean IoU = {ps["mean_iou"]:.3f}')
        lines.append('')
        for pi, gi, iou in ps['matched_pairs'][:max_listed_pairs]:
            pred = ps['pred_bboxes'][pi]
            gt = ps['gt_bboxes'][gi]
            dyaw_rad = (pred[6] - gt[6] + np.pi) % (2 * np.pi) - np.pi
            dyaw_deg = float(np.degrees(np.abs(dyaw_rad)))
            lines.append(f'  P{pi:2d}->G{gi:2d}  IoU={iou:.3f}  d_yaw={dyaw_deg:5.1f}deg')
        if len(ps['matched_pairs']) > max_listed_pairs:
            lines.append(f'  ...({len(ps["matched_pairs"]) - max_listed_pairs} more)')
    else:
        lines.append('No matched pairs.')

    if ps['unmatched_gt'] > 0 or ps['unmatched_pred'] > 0:
        lines.append('')
        lines.append(f'Unmatched: {ps["unmatched_gt"]} GT, {ps["unmatched_pred"]} Pred')
    ...
    if show_box_values:
        ...
        n_pred = ps['n_pred']
        if n_pred > 0:
            lines.append(f'Pred (showing {min(n_pred, max_listed_boxes)}/{n_pred}):')
            lines.append('   #   cx     cy     cz   sx   sy   sz   yaw   c')
            for i in range(min(n_pred, max_listed_boxes)):
                b = ps['pred_bboxes'][i]
                c = ps['pred_confidences'][i] if i < len(ps['pred_confidences']) else 0.0
                lines.append(f'  P{i:2d} {b[0]:+.2f} {b[1]:+.2f} {b[2]:+.2f} '
                             f'{b[3]:.2f} {b[4]:.2f} {b[5]:.2f} '
                             f'{math.degrees(float(b[6])):+5.0f} {c:.2f}')
            if n_pred > max_listed_boxes:
                lines.append(f'  ...({n_pred - max_listed_boxes} more)')
        # GT values
        n_gt = ps['n_gt']
        if n_gt > 0:
            lines.append('')
            lines.append(f'GT (showing {min(n_gt, max_listed_boxes)}/{n_gt}):')
            lines.append('   #   cx     cy     cz   sx   sy   sz   yaw')
            for i in range(min(n_gt, max_listed_boxes)):
                b = ps['gt_bboxes'][i]
                lines.append(f'  G{i:2d} {b[0]:+.2f} {b[1]:+.2f} {b[2]:+.2f} '
                             f'{b[3]:.2f} {b[4]:.2f} {b[5]:.2f} '
                             f'{math.degrees(float(b[6])):+5.0f}')
            if n_gt > max_listed_boxes:
                lines.append(f'  ...({n_gt - max_listed_boxes} more)')

    return '\n'.join(lines)


def format_per_sample_text_compact(ps, iou_thresholds=(0.25, 0.5, 0.75)):
    """Compact metrics text used in `grid_comparison/` cells.

    Shows only F1@thresholds, mean IoU, and center error.
    """
    iou_thresholds = list(iou_thresholds)
    width = 22
    lines = []
    ...
    lines.append('3D OBB F1:')
    for thresh in iou_thresholds:
        m = ps['per_threshold'][thresh]
        lines.append(f'  @{thresh:.2f}  =  {m["f1"]:.3f}')

    lines.append('-' * width)
    lines.append(f'mean IoU   = {ps["mean_iou"]:.3f}')

    ce_norm = ps.get('mean_center_error_3d', 0.0)
    ce_m = ps.get('mean_center_error_3d_m')
    if ce_m is not None:
        lines.append(f'center err = {ce_norm:.3f}')
        lines.append(f'           ({ce_m * 100:.1f} cm)')
    else:
        lines.append(f'center err = {ce_norm:.3f}')

    if ps.get('mean_rot_error_deg', 0.0) > 0:
        lines.append(f'rot err    = {ps["mean_rot_error_deg"]:.1f} deg')

    return '\n'.join(lines)


def visualize_grid_sampling(results, output_dir, per_page=12,
                             iou_thresholds=(0.25, 0.5, 0.75)):
    """Save samples as paginated grids with per-sample 3D OBB metrics text alongside.

    Layout per page: rows x (logical cells per row), each logical cell = (plot, text)
    side by side. Default `per_page=12` means 6 rows x 2 logical cells per row, with
    actual axes grid 6 rows x 4 cols (plot, text, plot, text).
    """
    grid_dir = os.path.join(output_dir, 'grid_sampling')
    os.makedirs(grid_dir, exist_ok=True)

    total = len(results)
    num_pages = math.ceil(total / per_page)

    # 2 logical cells per row works nicely for the text width
    cells_per_row = 2
    rows = int(math.ceil(per_page / cells_per_row))
    ncols_axes = cells_per_row * 2  # (plot, text) pair per logical cell
    width_ratios = [3, 4] * cells_per_row  # plot:text per logical cell
    cell_h = 4.0  # inches per row

    for page in tqdm(range(num_pages), desc='Visualizing grid_sampling'):
        start = page * per_page
        ...
        fig, axes = plt.subplots(
            rows, ncols_axes,
            figsize=(sum(width_ratios), cell_h * rows),
            dpi=150,
        ...
        for idx in range(n):
            ...
            col_pair = idx % cells_per_row
            ax_plot = axes[row, col_pair * 2]
            ax_text = axes[row, col_pair * 2 + 1]

            # ---- bbox plot (top-down) with index labels ----
            for pi, (pred, conf) in enumerate(zip(r['pred_bboxes'], r['pred_confidences'])):
                _draw_rotated_box(ax_plot, pred[0], pred[1], pred[3], pred[4], pred[6],
                                  color='red', alpha=0.3,
                                  label='Pred' if pi == 0 else None)
                ax_plot.text(pred[0], pred[1], f'P{pi}', ha='center', va='center',
                             fontsize=6, color='darkred', weight='bold')
            for gi, gt in enumerate(r['gt_bboxes']):
                _draw_rotated_box(ax_plot, gt[0], gt[1], gt[3], gt[4], gt[6],
                                  color='green', alpha=0.4,
                                  label='GT' if gi == 0 else None)
                ax_plot.text(gt[0], gt[1], f'G{gi}', ha='center', va='center',
                             fontsize=6, color='darkgreen', weight='bold')
            ax_plot.set_xlim(-0.55, 0.55)
            ax_plot.set_ylim(-0.55, 0.55)
            ax_plot.set_aspect('equal')
            short_id = r['sample_id'].split('/')[-1][:32]
            ax_plot.set_title(f'{short_id}', fontsize=7)
            ax_plot.grid(True, alpha=0.2)
            ax_plot.tick_params(labelsize=5)
            if idx == 0:
                ax_plot.legend(fontsize=6, loc='upper right')

            # ---- text panel: per-sample 3D OBB metrics ----
            ps = compute_per_sample_metrics_obb(r, iou_thresholds)
            text = format_per_sample_text(ps, iou_thresholds)
            ax_text.text(0.0, 1.0, text, va='top', ha='left',
        ...
        for idx in range(n, rows * cells_per_row):
            row = idx // cells_per_row
            col_pair = idx % cells_per_row
        ...
        fig.suptitle(
            f'Grid Sampling with 3D OBB Metrics  (page {page}, samples {start}-{end-1})',
            fontsize=10,
        )
        plt.tight_layout()
        plt.savefig(os.path.join(grid_dir, f'grid_sampling_{page}.png'),
                    bbox_inches='tight')
        plt.close(fig)

    print(f'Saved {num_pages} grid_sampling pages ({total} samples) to {grid_dir}')
...
def visualize_grid(results, output_dir, per_page=12, data_dir=None,
                   iou_thresholds=(0.25, 0.5, 0.75)):
    """Save samples as paginated grids in `grid_comparison/`, with each cell laid out
    as (bbox plot, metrics text) side by side.

    Per-cell text: F1@{0.25,0.50,0.75}, mean IoU, mean center error (cm if data_dir
    is provided so per-scene normalization scale can be loaded).
    """
    grid_dir = os.path.join(output_dir, 'grid_comparison')
    os.makedirs(grid_dir, exist_ok=True)

    # Optional: load per-sample normalization scale for meter-unit center error
    scale_cache = {}
    if data_dir is not None:
        for r in results:
            sid = r['sample_id']
    ...
    # 3 logical (plot, text) cells per row -> 4 rows of 3 cells = 12 per page
    cells_per_row = 3
    rows = int(math.ceil(per_page / cells_per_row))
    ncols_axes = cells_per_row * 2  # plot + text per logical cell
    width_ratios = [3, 2] * cells_per_row
    cell_h = 4.0
    ...
    for page in tqdm(range(num_pages), desc='Visualizing grid'):
        ...
        for idx in range(n):
            ...
            for gi, gt in enumerate(r['gt_bboxes']):
                _draw_rotated_box(ax_plot, gt[0], gt[1], gt[3], gt[4], gt[6],
                                  color='green', alpha=0.4,
                                  label='GT' if gi == 0 else None)
            ax_plot.set_xlim(-0.55, 0.55); ax_plot.set_ylim(-0.55, 0.55)
            ax_plot.set_aspect('equal')
            short_id = r['sample_id'].split('/')[-1][:30]
            ax_plot.set_title(f'{short_id}', fontsize=7)
            ax_plot.grid(True, alpha=0.2); ax_plot.tick_params(labelsize=5)
            if idx == 0:
                ax_plot.legend(fontsize=6, loc='upper right')

            # ---- compact metrics text panel on the right ----
            scale = scale_cache.get(r['sample_id'])
            ps = compute_per_sample_metrics_obb(r, iou_thresholds, scale=scale)
            text = format_per_sample_text_compact(ps, iou_thresholds)
            ax_text.text(0.02, 0.98, text, va='top', ha='left',
        ...
        fig.suptitle(f'Grid Comparison with 3D OBB Metrics  '
                     f'(page {page}, samples {start}-{end-1})',
                     fontsize=10)
        plt.tight_layout()
        plt.savefig(os.path.join(grid_dir, f'grid_comparison_{page}.png'),
                    bbox_inches='tight')
        plt.close(fig)
...
def main():
    ...
    ckpt_tag = f'ckpt_step{step_match.group(1)}_obb_grid' if step_match else f'ckpt_{ckpt_name}_obb_grid'
    ...
    visualize_grid(results, args.output_dir, per_page=12, data_dir=args.data_dir)
    visualize_heatmap_grid(results, args.output_dir)
    visualize_concat(results, args.output_dir, num_samples=args.num_vis)

    # Per-sample 3D OBB metrics annotated grid (more detailed view)
    visualize_grid_sampling(results, args.output_dir, per_page=12)
